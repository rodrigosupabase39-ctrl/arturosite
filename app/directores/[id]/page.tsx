...
export default function DirectorPage() {
  ...
  const { data: director, isLoading, error } = useTalento('directores', id);
      ...
      <>
        ...
        <Container maxWidth="lg" sx={{ py: 8, textAlign: 'center' }}>
          <Typography variant="h5" color="error">
            Error al cargar el director. Por favor, intenta nuevamente.
          </Typography>
        </Container>
      </>
    );
  ...
  try {
    if (Array.isArray(director.imagenes_urls)) {
      imagenesArray = director.imagenes_urls.filter(img => img && img.trim() !== '');
    } else if (typeof director.imagenes_urls === 'string') {
      const parsed = JSON.parse(director.imagenes_urls);
  ...
  try {
    if (Array.isArray(director.bloques)) {
      bloques = director.bloques;
    } else if (typeof director.bloques === 'string') {
      bloques = JSON.parse(director.bloques);
    }
  ...
  if (director.imagen_principal_url && director.imagen_principal_url.trim() !== '') {
    if (!todasLasImagenes.includes(director.imagen_principal_url)) {
      todasLasImagenes.unshift(director.imagen_principal_url);
    } else {
      todasLasImagenes = todasLasImagenes.filter(img => img !== director.imagen_principal_url);
      todasLasImagenes.unshift(director.imagen_principal_url);
    }
  }
  
  // Si no hay ninguna imagen pero hay imagen_principal_url, usarla
  if (todasLasImagenes.length === 0 && director.imagen_principal_url && director.imagen_principal_url.trim() !== '') {
    todasLasImagenes = [director.imagen_principal_url];
  }
    ...
    <>
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              <img
                src={selectedImage}
                alt={`${director?.nombre} - Imagen ampliada`}
                style={{
                  width: 'auto',
                  height: 'auto',
                  maxWidth: '90vw',
      ...
      >
        <Box sx={{ position: 'relative', width: '100%', aspectRatio: '16/9' }}>
          ...
          {director?.video_url && (
            <iframe
              src={director.video_url.replace('watch?v=', 'embed/').split('&')[0]}
              ...
              title={`Video de ${director.nombre}`}
            />
          )}
        </Box>
      </Dialog>
