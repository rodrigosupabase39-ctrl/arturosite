...
export default function DirectoresPage() {
  const { data, isLoading, error } = useTalentosList('directores');
          ...
          {error && (
            <Box sx={{ textAlign: 'center', py: 8 }}>
              <Typography color="error">
                Error al cargar los directores. Por favor, intenta nuevamente.
              </Typography>
            </Box>
          )}
          ...
          {data && data.talentos && data.talentos.length > 0 && (
            <Grid container spacing={{ xs: 3, sm: 4, md: 4 }}>
              {data.talentos.map((director) => {
                ...
                const imagenPrincipal = director.imagen_principal_url || 
                  (imagenesArray && imagenesArray.length > 0 ? imagenesArray[0] : null);

                return (
                  <Grid size={{ xs: 6, sm: 4, md: 3, lg: 2 }} key={director.id}>
                    <Link
