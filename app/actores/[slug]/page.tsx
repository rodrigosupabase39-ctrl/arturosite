...
export default function ActorPage() {
  ...
  const { data: actor, isLoading, error } = useTalento('actores', slug);
      ...
      <>
        ...
        <Container maxWidth="lg" sx={{ py: 8, textAlign: 'center' }}>
          <Typography variant="h5" color="error">
            Error al cargar el actor. Por favor, intenta nuevamente.
          </Typography>
        </Container>
      </>
    );
  ...
  try {
    if (Array.isArray(actor.imagenes_urls)) {
      imagenesArray = actor.imagenes_urls.filter(img => img && img.trim() !== '');
    } else if (typeof actor.imagenes_urls === 'string') {
      const parsed = JSON.parse(actor.imagenes_urls);
  ...
  try {
    if (Array.isArray(actor.bloques)) {
      bloques = actor.bloques;
    } else if (typeof actor.bloques === 'string') {
      bloques = JSON.parse(actor.bloques);
    }
  ...
  if (actor.imagen_principal_url && actor.imagen_principal_url.trim() !== '') {
    if (!todasLasImagenes.includes(actor.imagen_principal_url)) {
      todasLasImagenes.unshift(actor.imagen_principal_url); // Agregar al inicio
    } else {
      // Si ya está en el array, moverla al inicio
      todasLasImagenes = todasLasImagenes.filter(img => img !== actor.imagen_principal_url);
      todasLasImagenes.unshift(actor.imagen_principal_url);
    }
  }
  
  // Si no hay ninguna imagen pero hay imagen_principal_url, usarla
  if (todasLasImagenes.length === 0 && actor.imagen_principal_url && actor.imagen_principal_url.trim() !== '') {
    todasLasImagenes = [actor.imagen_principal_url];
  }
  ...
  return (
    <>
      ...
      {(actor as any).imagen_portada_url ? (
        <Box 
          sx={{ 
            width: '100%',
            height: '460px',
        ...
        >
          <img
            src={(actor as any).imagen_portada_url}
            alt={`Portada de ${actor.nombre}`}
            style={{
              width: '100%',
              height: '100%',
              objectFit: 'cover',
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              <img
                src={selectedImage}
                alt={`${actor?.nombre} - Imagen ampliada`}
                style={{
                  width: 'auto',
                  height: 'auto',
                  maxWidth: '90vw',
      ...
      >
        <Box sx={{ position: 'relative', width: '100%', aspectRatio: '16/9' }}>
          ...
          {actor?.video_url && (
            <iframe
              src={actor.video_url.replace('watch?v=', 'embed/').split('&')[0]}
