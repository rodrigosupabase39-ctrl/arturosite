...
export default function GuionistasPage() {
  const { data, isLoading, error } = useTalentosList('guionistas');
          ...
          {error && (
            <Box sx={{ textAlign: 'center', py: 8 }}>
              <Typography color="error">
                Error al cargar los guionistas. Por favor, intenta nuevamente.
              </Typography>
            </Box>
          )}
          ...
          {data && data.talentos && data.talentos.length === 0 && (
            <Box sx={{ textAlign: 'center', py: 8 }}>
              <Typography variant="h6" color="text.secondary">
                No hay guionistas registrados aún.
              </Typography>
            </Box>
          )}
              ...
              {data.talentos.map((guionista) => {
                ...
                try {
                  if (Array.isArray(guionista.imagenes_urls)) {
                    imagenesArray = guionista.imagenes_urls;
                  } else if (typeof guionista.imagenes_urls === 'string') {
                    imagenesArray = JSON.parse(guionista.imagenes_urls);
                  }
                } catch (error) {
                ...
                const imagenPrincipal = guionista.imagen_principal_url || 
