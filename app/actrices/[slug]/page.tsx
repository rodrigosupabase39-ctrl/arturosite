...
export default function ActrizPage() {
      ...
      <>
        ...
        <Container maxWidth="lg" sx={{ py: 8, textAlign: 'center' }}>
          <Typography variant="h5" color="error">
            Error al cargar la actriz. Por favor, intenta nuevamente.
          </Typography>
        </Container>
      </>
    );
  ...
  try {
    if (Array.isArray(actriz.imagenes_urls)) {
      imagenesArray = actriz.imagenes_urls.filter(img => img && img.trim() !== '');
    } else if (typeof actriz.imagenes_urls === 'string') {
      const parsed = JSON.parse(actriz.imagenes_urls);
  ...
  try {
    if (Array.isArray(actriz.bloques)) {
      bloques = actriz.bloques;
    } else if (typeof actriz.bloques === 'string') {
      bloques = JSON.parse(actriz.bloques);
    }
  ...
  if (actriz.imagen_principal_url && actriz.imagen_principal_url.trim() !== '') {
    if (!todasLasImagenes.includes(actriz.imagen_principal_url)) {
      todasLasImagenes.unshift(actriz.imagen_principal_url);
    } else {
      todasLasImagenes = todasLasImagenes.filter(img => img !== actriz.imagen_principal_url);
      todasLasImagenes.unshift(actriz.imagen_principal_url);
    }
  }
  
  // Si no hay ninguna imagen pero hay imagen_principal_url, usarla
  if (todasLasImagenes.length === 0 && actriz.imagen_principal_url && actriz.imagen_principal_url.trim() !== '') {
    todasLasImagenes = [actriz.imagen_principal_url];
  }
    ...
    <>
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              <img
                src={selectedImage}
                alt={`${actriz?.nombre} - Imagen ampliada`}
                style={{
                  width: 'auto',
                  height: 'auto',
                  maxWidth: '90vw',
      ...
      >
        <Box sx={{ position: 'relative', width: '100%', aspectRatio: '16/9' }}>
          ...
          {actriz?.video_url && (
            <iframe
              src={actriz.video_url.replace('watch?v=', 'embed/').split('&')[0]}
