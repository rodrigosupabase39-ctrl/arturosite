...
export default function TalentoSub18Page() {
  const params = useParams();
  const slug = params?.slug as string;
  
  const { data: talento, isLoading, error } = useTalento('talentos-sub-18', slug);
  ...
  if (error || !talento) {
    console.error('[TalentoSub18Page] Error:', { error, slug, hasTalento: !!talento });
    return (
      <>
        <Header />
        <Container maxWidth="lg" sx={{ py: 8, textAlign: 'center' }}>
          <Typography variant="h5" color="error">
            Error al cargar el talento. Por favor, intenta nuevamente.
          </Typography>
          {error && (
  ...
  try {
    if (Array.isArray(talento.imagenes_urls)) {
      imagenesArray = talento.imagenes_urls.filter(img => img && img.trim() !== '');
    } else if (typeof talento.imagenes_urls === 'string') {
      const parsed = JSON.parse(talento.imagenes_urls);
  ...
  try {
    if (Array.isArray(talento.bloques)) {
      bloques = talento.bloques;
    } else if (typeof talento.bloques === 'string') {
      bloques = JSON.parse(talento.bloques);
    }
  ...
  if (talento.imagen_principal_url && talento.imagen_principal_url.trim() !== '') {
    if (!todasLasImagenes.includes(talento.imagen_principal_url)) {
      todasLasImagenes.unshift(talento.imagen_principal_url);
    } else {
      todasLasImagenes = todasLasImagenes.filter(img => img !== talento.imagen_principal_url);
      todasLasImagenes.unshift(talento.imagen_principal_url);
    }
  }
  
  // Si no hay ninguna imagen pero hay imagen_principal_url, usarla
  if (todasLasImagenes.length === 0 && talento.imagen_principal_url && talento.imagen_principal_url.trim() !== '') {
    todasLasImagenes = [talento.imagen_principal_url];
  }
  ...
  return (
    <>
      ...
      {(talento as any).imagen_portada_url ? (
        <Box 
          sx={{ 
            width: '100%',
            height: '460px',
        ...
        >
          <img
            src={(talento as any).imagen_portada_url}
            alt={`Portada de ${talento.nombre}`}
            style={{
              width: '100%',
              height: '100%',
              objectFit: 'cover',
        ...
        {selectedImage && (
          ...
          >
            ...
            >
              <img
                src={selectedImage}
                alt={`${talento?.nombre} - Imagen ampliada`}
                style={{
                  width: 'auto',
                  height: 'auto',
                  maxWidth: '90vw',
      ...
      >
        <Box sx={{ position: 'relative', width: '100%', aspectRatio: '16/9' }}>
          ...
          {talento?.video_url && (
            <iframe
              src={talento.video_url.replace('watch?v=', 'embed/').split('&')[0]}
